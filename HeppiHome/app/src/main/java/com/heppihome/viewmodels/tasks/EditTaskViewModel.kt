...
@HiltViewModel
class EditTaskViewModel @Inject constructor(private val rep : HomeRepository)
    : ViewModel() {
    ...
    fun setTask(task : Task) {
        ...
        calendar.time = task.deadline.toDate()
        _hours.value = DateUtil.formatHours(
            calendar.get(Calendar.HOUR_OF_DAY),
            calendar.get(Calendar.MINUTE)
        )
        ...
        _name.value = task.text

        _points.value = task.points.toString()
        _users.value = task.users
    ...
    fun getTask() : Task {
        return Task(_name.value, done, Timestamp(calendar.time),
            _users.value, NumberUtil.verifyInput(_points.value), id)
    }

    fun updateTask() {
        val task = getTask()
        viewModelScope.launch {
            rep.updateTask(task).collect {  }
        }
    }
